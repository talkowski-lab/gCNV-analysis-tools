...
DEFAULT_DCR_EVIDENCE <- tibble(M = NA_real_,
                               MF = NA_real_,
                               MM = NA_real_,
                               MD = NA_real_,
...
denovo_from_ovp <- function(child, father, mother, bins) {
    ...
    filter(child, inheritance == "denovo")
}

add_parent_batch <- function(child, all_calls) {
    batches <- select(all_calls, sample, batch) |>
        distinct() |>
        mutate(
            paternal_id = sample,
            maternal_id = sample,
    ...
    child <- select(batches, paternal_id, paternal_batch) |>
        inner_join(child, by = "paternal_id", relationship = "one-to-many")
    child <- select(batches, maternal_id, maternal_batch) |>
        inner_join(child, by = "maternal_id", relationship = "one-to-many")
...
autosome_denovo <- function(calls, bins, ped, dcrs, nproc) {
    calls <- filter(calls, !grepl("X|Y", chr))
    ped <- filter(ped, sample_id %in% calls$sample) |>
        filter(paternal_id %in% calls$sample) |>
        filter(maternal_id %in% calls$sample)
    ...
    dn <- denovo_from_ovp(child_calls,
                          filter(calls, sample %in% ped$paternal_id),
                          filter(calls, sample %in% ped$maternal_id),
                          bins)
...
chrx_denovo <- function(calls, bins, ped, dcrs, nproc) {
    ...
    ped <- filter(ped, sample_id %in% calls$sample, !is.na(sex))

    # Predict the sex of all samples ------------------------------------------
    log_info("validating sex of samples")
    predicted_sex <- predict_sex(calls[, c("sample", "batch")], dcrs, nproc)
    ...
    calls <- filter(calls, sample %in% pass_sex$sample)
    ped <- filter(ped, sample_id %in% calls$sample) |>
        filter(paternal_id %in% calls$sample) |>
        filter(maternal_id %in% calls$sample)
    ...
    log_info("making initial de novo predictions based on CNV overlap")
    dn <- denovo_from_ovp(child_calls,
                          filter(calls, sample %in% ped$paternal_id, grepl("X", chr)),
                          filter(calls, sample %in% ped$maternal_id, grepl("X", chr)),
                          bins)
    log_info(sprintf("found %d de novo events based on overlap", nrow(dn)))
